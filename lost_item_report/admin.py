...
from lost_found.settings import NEW_LOST_FOUND_ITEM_TEMPLATE
from lost_item_report.utils import send_email_functionality
...
class ReportLostItemAdmin(admin.ModelAdmin):
    def save_model(self, request, obj, form, change):
        try:
            ...
            emails = ReportLostItem.objects.filter(item_category=obj.item_category).values_list(
                "reported_by__email", flat=True).distinct()
            send_email_functionality(
                emails=emails, item_name=obj.item_name, template_name=NEW_LOST_FOUND_ITEM_TEMPLATE,
                subject="New Lost Item Reported", model_version=obj.model_version,
                description=obj.description, location=obj.location,
                date_lost=obj.date_lost,
            )
...
class FoundItemAdmin(admin.ModelAdmin):
    ...
    def image_viewer_function(self, single_db_obj):
        return format_html(f'<img src="{single_db_obj.image.url}" width="auto"/>')
